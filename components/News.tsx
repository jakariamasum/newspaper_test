...
const News: React.FC<NewssProps> = ({ title, link, limit, box, style, item }) => {
  ...
  const prevImage = () => {
    setCurrentImage((prevIndex) => (prevIndex - 1 + items.length) % items.length);
  };
  ...
  return (
    ...
    {style === 1 && (
      <div className='flex items-center justify-between mb-2 px-2 py-1 border-b-2 border-main bg-white'>
        <h2 className="text-xl font-bold">{title} {box}</h2>
            ...
            <>
              {item.map((categoryItem, index) => (
                <button
                  ...
                  className={index === activeCategory ? 'bg-main text-white px-2 py-0 rounded-sm' : ''}
                >
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-1'>
        {item[activeCategory].post.slice(0, limit).map((p, index) => {
          if (index === 0) {
            ...
            return (
              <Link href={p.link} key={index} className="col-span-3 bg-white p-2 flex items-center space-x-2">
                <Image
                  src={p.img || "/default.jpg"}
                  width={300}
                  height={300}
    ...
    {box === 2 && (
      <div className='grid grid-cols-1 gap-1'>
        {item[activeCategory].post.slice(0, limit).map((post, index) => (
          <Link href={post.link} className='bg-white p-2 flex items-center space-x-1' key={index}>
            <Image
      ...
      <div className='grid grid-cols-3 gap-2'>
        <div className='space-y-2 block'>
          {item[activeCategory].post.slice(0, 4).map((p, index) => (
            <Link href={p.link} className='bg-white p-1.5 flex items-center space-x-1' key={index}>
              <Image
                src={p.img || "/default.jpg"}
                width={300}
                height={300}
        ...
        {item[activeCategory].post.slice(5, 9).map((p, index) => (
            <Link href={p.link} className='bg-white p-1.5 flex items-center space-x-2' key={index}>
              <p className='line-clamp-2'>{p.title}</p>
              <Image
                src={p.img || "/default.jpg"}
                width={300}
    ...
    {box === 10 && (
      <div className='grid grid-cols-3 grid-flow-col gap-2'>
        {item[activeCategory].post.slice(0, 1).map((p, index) => (
          <Link href={p.link} className='bg-white h-full p-2 block' key={index}>
            <Image
              src={p.img || "/default.jpg"}
              width={600}
              height={600}
    ...
    {box === 11 && (
      <div className='flex flex-col md:flex-row gap-2'>
        {item[activeCategory].post.slice(0, 1).map((p, index) => (
          <Link href={p.link} className='w-full md:w-1/4 bg-white h-auto p-2 block' key={index}>
            <Image
              src={p.img || "/default.jpg"}
              width={600}
              height={600}
    ...
    {box === 12 && (
      <div className='flex md:flex-row flex-col gap-2'>
        ...
        <div className='w-full md:w-1/2'>
          {item[activeCategory].post.slice(0, 1).map((p, index) => (
            <Link href={p.link} className='bg-white p-2 block h-auto' key={index}>
              <Image
                src={p.img || "/default.jpg"}
                width={600}
                height={600}
                ...
                className='w-full h-96 object-cover'
              />
              <p className='line-clamp-2 mt-2 text-base leading-normal'>{p.title}</p>
            </Link>
          ))}
        </div>
    ...
    {box === 13 && (
      <div className='w-full flex flex-col md:flex-row gap-2'>
        {item[activeCategory].post.slice(0, 1).map((p, index) => (
            <Link href={p.link} className='w-full md:w-2/4	bg-white p-2 block h-auto' key={index}>
              <Image
                src={p.img || "/default.jpg"}
                width={600}
                height={600}
          ...
          {item[activeCategory].post.slice(1, limit).map((p, index) => (
            <Link href={p.link} className='bg-white p-2 flex items-center space-x-1' key={index}>
              <Image
                src={p.img || "/default.jpg"}
                width={600}
                height={600}
    ...
    {box === 14 && (
      <div className='w-full flex flex-col md:flex-row gap-2'>
        {item[activeCategory].post.slice(0, 1).map((p, index) => (
          ...
          {item[activeCategory].post.slice(1, limit).map((p, index) => (
            <Link href={p.link} className='bg-white p-2 space-y-1' key={index}>
              <Image
                src={p.img || "/default.jpg"}
                width={600}
                height={600}
      ...
      <div className="relative group">
        <div className="w-full bg-white p-2 relative h-40 md:h-96 flex items-center justify-center">
          ...
          <div className="absolute flex md:group-hover:flex md:hidden justify-between w-full items-center gap-2">
            <div onClick={prevImage} className="bg-black flex items-center justify-center opacity-50 w-8 md:h-16 h-10 text-white">
              <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
                <path fill="currentColor" fillRule="evenodd" d="m15 4l2 2l-6 6l6 6l-2 2l-8-8z"/>
              </svg>
            </div>
            <div onClick={nextImage} className="bg-black flex items-center justify-center opacity-50 w-8 md:h-16 h-10 text-white">
              <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24">
        ...
        <div className="absolute bottom-4 left-0 right-0 group-hover:flex hidden justify-center">
          {items.map((_, index) => (
            <div
              key={index}
              className={`w-2 h-2 rounded-full mx-1 cursor-pointer ${currentImage === index ? 'bg-black' : 'bg-gray-300'}`}
    ...
    {box === 17 && (
      <div className="relative group">
        <div ref={containerRef} className="flex overflow-hidden gap-2 transition-transform duration-500">
          {displayedItems.map((p, index) => (
            <div key={index} className={`w-full sm:w-3/6 md:w-1/${itemsToShow} transition-opacity duration-500`}>

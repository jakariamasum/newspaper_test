...
const Banners: React.FC<BannersProps> = ({ items, onChange }) => {
  ...
  const handleImageChange = (
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        const updatedBanners = banners.map((banner, i) =>
          i === index ? { ...banner, img: imgUrl } : banner
        );
        setBanners(updatedBanners);
        onChange(updatedBanners);
      };
  ...
  const addBanner = () => {
    const updatedBanners = [...banners, { img: "", title: "" }];
    setBanners(updatedBanners);
    onChange(updatedBanners);
  };
    ...
    <div>
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4">
        {banners.map((banner, index) => (
          <div
            key={index}
            className="bg-white w-full relative flex flex-col items-center"
          >
            ...
            <button
              ...
              className="text-white p-1 absolute right-2 top-2 bg-red-500/50 hover:bg-red-500"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="15"
